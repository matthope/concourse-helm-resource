...
setup_helm() {
  ...
  if [ "$init_server" = true ]; then
    ...
    if [ "$tls_enabled" = true ]; then
      ...
      helm init \
        --tiller-tls --tiller-tls-cert $tiller_cert_path --tiller-tls-key $tiller_key_path --tiller-tls-verify --tls-ca-cert $tiller_key_path \
    ...
    HELM_HOST=$(jq -r '.source.helm_host // ""' < "$1")
    export HELM_HOST
